...
#     print(addre_list)
    ...
    for addre in addre_list: 
        flow_sab = len(str(addre).split(';'))
        flow_list.append(flow_sab)
    #print(flow_list)
    #print('flow down')
    #
    ...
    for i in range(len(flow_list)):
        if dist_list[i] > 0:
            if orient_list[i] == 1:
                power1 = power1 + float(flow_list[i])*100/float(dist_list[i])
            elif orient_list[i] == 2:
                power2 = power2 + float(flow_list[i])*100/float(dist_list[i])
            elif orient_list[i] == 3:
                power3 = power3 + float(flow_list[i])*100/float(dist_list[i])
            elif orient_list[i] == 4:
                power4 = power4 + float(flow_list[i])*100/float(dist_list[i])
            elif orient_list[i] == 5:
                power5 = power5 + float(flow_list[i])*100/float(dist_list[i])
            elif orient_list[i] == 6:
                power6 = power6 + float(flow_list[i])*100/float(dist_list[i])
            elif orient_list[i] == 7:
                power7 = power7 + float(flow_list[i])*100/float(dist_list[i])
            else:
                power8 = power8 + float(flow_list[i])*100/float(dist_list[i])
...
def main():
    ...
    building_type = ['公交站','地铁站']
    t_type = building_type[0 ]
    ...
    for i in range(int(total_round)):
        if every_round*(i+1)-1 >= len(list_locations):
            list_o_location = getLat_Lng(list_locations[every_round*i:len(list_locations)+1],ak[i%4])
        if every_round*(i+1)-1 < len(list_locations):
            list_o_location = getLat_Lng(list_locations[every_round*i:every_round*(i+1)],ak[i%4])
            #print(o_location)
        ...
        for o_location in list_o_location:
            ...
            #print(o_location)
            url = "http://api.map.baidu.com/place/v2/search?query=" + t_type + "&location=" + str(o_location[0])+ ',' + str(o_location[1]) + "&radius=" + str(radius) + "&output=json&sort_name:distance|sort_rule:1&ak=" + ak[i%4]
            #print(url)
            r = requests.get(url)
            #print(r)
            data = r.json()
            ...
            if results == []:
                ...
                public_transportation_addname = get_everything(results,o_location,pt_path,ak[i%4])
                public_transportation_addname.insert(0, lona)
                public_transportation.append(public_transportation_addname)
            #print(public_transportation)
